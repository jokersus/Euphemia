...
class ECommandHandler {
	...
	loadModules() {
		...
		modules.forEach(m => {
			...
			this.client.once('ready', async () => {
				Promise.all(parsedCommands.filter(c => c.slash).map(async c => {
					await this.client.guilds.resolve('344556222755766283').commands.create({
						name: c.aliases[0],
						description: c.description.content,
						options: c.interactionParser.parseInteractionOptions()
					});
				})).catch(console.error);
			});
		});
	...
	async handleInteraction(interaction) {
		...
		if (!command) {
			throw 'What the absolute fuck?';
		}

		try {
			await command.executeInteraction(interaction, interaction.options);
		} catch (err) {
